...
逃逸分析说的是这样的一种情况: JVM通过分析对象的动态作用域, 当一个对象在方法中定义以后, 它可能会被外部的方法引用, 例如方法中的定义的对象被传递到其他方法中, 这称为方法逃逸. 如果被其他的线程访问到, 还有可能发生线程逃逸.

如果能够证明一个对象不会发生方法逃逸和线程逃逸则可以进行一些优化:
- 栈上分配. 直接在栈空间上分配对象, 而不在堆中分配.
- 同步消除. 如果可以证明一个对象不会发生线程逃逸则可以去掉同步措施.
- 标量替换. 把对象这种聚合量分解为标量(int, char).

开启逃逸分析: `-XX:+DoEscapeAnalysis`

### TLAB

TLAB是线程本地分配的缩写. 这块空间分配在Eden去, 默认使用1%的空间. 使用线程本地分配的话没有锁的竞争. 会比直接在eden去分配更快. 
