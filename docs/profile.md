...
- 熟练使用Java和Scala, 了解Golang，Python和JavaScript.
...
- 监控系统数据采集结合了推和拉两种方式。应用的基础指标，包括jvm的信息，服务接口调用，线程数等等，使用telegraf定时扫描spring boot的metrics接口。对于业务指标我们自研了数据上报和分析系统。

- 数据上报模块服务使用gRPC和Spring Boot开发，对外提供protobuf和http两种协议。摩拜绝大多数应用使用Java开发，所以，我们封装了Java语言的SDK，同时还封装了Spring Starter，方便业务开发者使用。SDK基于gRPC实现，传输效率和性能更高。对于其它非Java应用，可以使用http接口上报数据。数据上报服务端在收到了数据之后，直接丢到了kafka。另外还实现了鉴权和流量控制功能，只有持有正确key的SDK才能上报数据，而且流量不能超过预配置上限。

- 数据上报服务和SDK端有定时的心跳。一方面可以用于检测应用的健康状况，另一方面还可以和服务端同步配置信息。动态的调整客户端指标聚合的时间维度。

2. 策略模块使用golang开发，思路借鉴了小米的Open-Falcon，在Open-Falcon的基础上实现了同比函数。

3. 报警模块目前实现了四种报警通道：企业微信，短信，语音，邮件。同时报警模块还支持报警关闭和基
